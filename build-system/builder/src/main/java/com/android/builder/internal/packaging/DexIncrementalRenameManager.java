...
class DexIncrementalRenameManager implements Closeable {
    ...
    private static class DexNameComparator implements Comparator<RelativeFile> {
        ...
        /**
         * Get the index from the dex filename inside the APK.
         * For example, if the dex filename is "classes2.dex", then return 2
         * if the dex filename is "classes.dex", then return 1
         *
         * @param dexName the dex filename inside the APK
         * @return the dex index
         */
        private int getDexIndex(@NonNull String dexName) {
            int index = 1;
            if (!SdkConstants.FN_APK_CLASSES_DEX.equals(dexName)) {
                int eIndex = dexName.indexOf(SdkConstants.DOT_DEX);
