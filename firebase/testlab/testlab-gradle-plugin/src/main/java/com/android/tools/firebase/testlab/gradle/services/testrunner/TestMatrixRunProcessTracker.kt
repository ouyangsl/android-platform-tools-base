...
class TestMatrixRunProcessTracker(
  ...
  @VisibleForTesting private val checkTestStateWaitMs: Long = DEFAULT_CHECK_STATE_WAIT_MS,
  @VisibleForTesting private val logger: Logger = DEFAULT_LOGGER,
) {
  ...
  fun waitForTestResults(deviceName: String, testRunMatrix: TestMatrix): TestMatrix {
    ...
    while (true) {
      val latestTestMatrix = testingManager.getTestMatrix(projectName, testRunMatrix)
      ...
      if (printResultsUrl) {
        ...
        if (!resultsUrl.isNullOrBlank()) {
          val resultDetailsUrl =
            if (resultsUrl.endsWith("details")) {
              resultsUrl
            } else {
              "$resultsUrl/details"
            }
