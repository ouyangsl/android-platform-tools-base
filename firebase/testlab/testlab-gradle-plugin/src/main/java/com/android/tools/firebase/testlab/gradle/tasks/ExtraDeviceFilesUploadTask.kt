...
abstract class ExtraDeviceFilesUploadTask : DefaultTask() {
  ...
  @get:Input abstract val extraFiles: MapProperty<String, String>
  ...
  fun validateOrUploadExtraFiles() {
    ...
    extraFiles.get().forEach { devicePath, filePath ->
      ...
      storageObject?.apply { extraDeviceFiles.add(devicePath, this) }
    }
  ...
  private fun isGoogleStorageLink(filePath: String) = filePath.startsWith("gs://")
}
