 ...
 */
class TestResultProcessor(private val directoriesToPull: List<String>) {
    ...
    TestSuiteResult.newBuilder()
      .apply {
        ...
        executionStep.testExecutionStep.testSuiteOverviews.also { overviews ->
          testSuiteMetaData =
            TestSuiteMetaData.newBuilder()
              .apply {
                testSuiteName = executionStep.name
                scheduledTestCaseCount = overviews?.sumOf { it?.totalCount ?: 0 } ?: 0
              }
              .build()

          overviews?.get(0)?.xmlSource?.also { xml -> addTestResultXmlFileArtifact(this, xml) }
        }
        ...
        thumbnails?.also { addThumbnailOutputArtifacts(this, it) }
  ...
  private fun addTestResultXmlFileArtifact(
    ...
    builder.apply {
      xmlSource.fileUri?.also { path ->
        if (path.isNotBlank()) {
          addOutputArtifact(createOutputArtifact(label = "firebase.xmlSource", path = path))
        }
      }
    }
  }
  ...
  private fun addTestIssues(builder: TestSuiteResult.Builder, testIssues: List<TestIssue>) {
    builder.apply {
          ...
          TestResult.newBuilder()
            .apply {
              ...
              if (testMethod != null || qualifiedName != null) {
                ...
                case.toolOutputs
                  ?.asSequence()
                  ?.map { output -> output.output?.fileUri ?: "" }
                  ?.filter { uri -> uri.endsWith("logcat") }
                  ?.forEach { uri ->
                    testRunStorage
      ...
      builder.apply {
        platformErrorBuilder.addErrorsBuilder().apply {
          summaryBuilder.apply {
            ...
            errorMessage = getInvalidMatrixDetailsErrorMessage(invalidMatrixDetails)
  ...
  private fun testStatusFromSummary(firebaseTestSummary: String?) =
    when (firebaseTestSummary) {
...
private fun createOutputArtifact(label: String, path: String, mimeType: String? = null) =
  Artifact.newBuilder()
    .apply {
      this.label =
        Label.newBuilder()
          ...
          .build()
      sourcePath = Path.newBuilder().apply { this.path = path }.build()
      type = ArtifactType.TEST_DATA
      mimeType?.also { this.mimeType = it }
    }
    .build()

internal fun TestSuiteResult.passed(): Boolean {
  val suitePassed = testStatus.isPassedOrSkipped()
  val allTestsPass = testResultList.all { case -> case.testStatus.isPassedOrSkipped() }
