...
class TestRunner(
          ...
          testSuiteElements.forEach { testSuite ->
            ...
            val propertyElement =
              if (propertyNode == null) {
                document.createElement("properties").also { testSuite.appendChild(it) }
              } else {
                propertyNode as Element
              }

            propertyElement.appendChild(
              document.createElement("property").apply {
  ...
  fun runTests(
    ...
    if (resultProtos.isNotEmpty()) {
      ...
      mergedTestResultPbFile.outputStream().use { resultsMerger.result.writeTo(it) }
    }
  ...
  private fun handleTestSuiteResult(
    ...
    return if (results != null) {
      ...
      testResultProcessor.toUtpResult(testMatrix.invalidMatrixDetails)
    }
  }
