...
class TestRunnerTest {
  ...
  @Mock lateinit var projectSettings: ProjectSettings

  @Mock lateinit var toolResults: ToolResultsManager

  @Mock lateinit var testing: TestingManager

  @Mock lateinit var storage: StorageManager

  @Mock lateinit var mockTestRunStorage: TestRunStorage

  @Mock lateinit var testResultProcessor: TestResultProcessor

  @Mock lateinit var testMatrixGenerator: TestMatrixGenerator

  @Mock lateinit var matrixRunTracker: TestMatrixRunProcessTracker

  @Mock lateinit var deviceFileManager: DeviceInfoFileManager

  @Mock lateinit var xmlHandler: TestResultsXmlHandler

  @Mock lateinit var device: TestDeviceData

  @Mock lateinit var staticTestData: StaticTestData

  @Mock lateinit var mockGeneratedMatrix: TestMatrix

  @Mock lateinit var mockUpdatedMatrix: TestMatrix

  @Mock lateinit var mockResultsMatrix: TestMatrix
  ...
  @Mock lateinit var toolResultsStep: ToolResultsStep

  @Mock lateinit var mockStep: Step

  @Mock lateinit var mockTestExecutionStep: TestExecutionStep

  @Mock lateinit var mockUtpResult: TestSuiteResult

  @Mock lateinit var mockTestSuiteMerger: UtpTestSuiteResultMerger

  @Captor lateinit var requestInfoCaptor: ArgumentCaptor<ToolResultsManager.RequestInfo>
  ...
  fun setup() {
    ...
    staticTestData.apply {
      ...
      `when`(testedApkFinder).thenReturn { _ -> listOf(testedApk) }
    }
    ...
    toolResults.apply { `when`(getOrCreateHistory(any(), any())).thenReturn("history_id") }
  ...
  private fun verifyTestRun(
    ...
    verify(matrixRunTracker).waitForTestResults("device_name", mockUpdatedMatrix)
  ...
  fun test_runTests_testMatrixFailure() {
    ...
    verifyTestRun(basicVerifyToolResults = false, basicVerifyTestResultProcessor = false)
  ...
  fun test_runTests_testSuiteOverview() {
    ...
    verifyTestRun(basicVerifySuiteOverview = false)
