...
class TestRunTaskActionTest {
  ...
  @Mock lateinit var parameters: DeviceTestRunParameters<DeviceTestRunInput>

  @Mock lateinit var mockTestRunData: TestRunData

  @Mock lateinit var testLabBuildService: TestLabBuildService
  ...
  fun setup() {
    ...
    val setupResultDir =
      project.layout.buildDirectory.dir("setupOutput").get().apply {
        asFile.mkdirs()
        file("myManagedDevice.json").asFile.writeText(Gson().toJson(model))
      }

    setupResultProperty = project.objects.directoryProperty().apply { set(setupResultDir) }

    // Create the extra device files urls file
    extraFileUrls =
      project.layout.buildDirectory.file("urls").get().also {
        ExtraDeviceFilesManager()
          .apply {
    ...
    val input =
      project.objects.newInstance(DeviceTestRunInput::class.java).apply {
        device.set("Pixel3")
        apiLevel.set(33)
        locale.set("fr-CA")
    ...
    `when`(
        testLabBuildService.runTestsOnDevice(
          deviceName = any(),
          deviceId = any(),
          deviceApiLevel = any(),
          deviceLocale = any(),
    ...
    testResults = listOf(FtlTestRunResult(true, null))
  }
  ...
  fun test_runTests() {
    ...
    verify(testLabBuildService)
      .runTestsOnDevice(
        ...
        eq(":"),
        eq("debug"),
        eq(mapOf("location1" to "gs://bucket/path", "location2" to "gs://bucket/other/path")),
      )
  }
