...
class StorageManagerTest {
  ...
  @Mock lateinit var cloudStorageClient: Storage

  @Mock lateinit var mockObjects: Storage.Objects

  @Mock lateinit var mockInsert: Storage.Objects.Insert

  @Mock lateinit var mockGet: Storage.Objects.Get
  ...
  fun testUpload() {
    ...
    val file = temporaryFolderRule.newFile("World").apply { writeText("aabbcc") }

    val storageManager = StorageManager(cloudStorageClient)

    val result = storageManager.uploadFile(file, "some-bucket", "Hello_")
  ...
  fun testDownloadFile_returnsNullWhenFailed() {
    ...
    `when`(mockGet.executeMediaAndDownloadTo(any())).thenAnswer { throw exception }
  ...
  fun testRetrieveOrUploadShared() {
    `when`(mockGet.execute()).thenAnswer { throw GoogleJsonResponseException(mock(), mock()) }
    ...
    val file = temporaryFolderRule.newFile("text").apply { writeText("aabbcc") }
