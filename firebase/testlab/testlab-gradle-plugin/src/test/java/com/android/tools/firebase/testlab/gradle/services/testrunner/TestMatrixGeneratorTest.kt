...
class TestMatrixGeneratorTest {
  ...
  @Mock lateinit var mockModel: AndroidModel

  @Mock lateinit var testData: StaticTestData

  @Mock lateinit var testRunStorage: TestRunStorage

  @Mock lateinit var testApk: StorageObject
  ...
  fun test_createTestMatrix_extraDeviceUrls() {
    ...
    verifyMatrix(
      matrix,
      testSpecSetupFilesToPush =
        listOf(
          DeviceFile().apply {
            regularFile =
              RegularFile().apply {
                content = FileReference().apply { gcsPath = "gs://link/to/storage" }
          ...
          DeviceFile().apply {
            regularFile =
              RegularFile().apply {
                content = FileReference().apply { gcsPath = "gs://same/storage/path" }
  ...
  fun verifyMatrix(
    ...
    environmentDevices: List<AndroidDevice> =
      listOf(
        AndroidDevice().apply {
    ...
    matrix.testSpecification.also { testSpec ->
      ...
      testSpec.androidInstrumentationTest.also { instrumentation ->
        assertThat(instrumentation.testApk.gcsPath).isEqualTo(testSpecInstrumentationTestApk)
    ...
    matrix.resultStorage.also { storage ->
      assertThat(storage.googleCloudStorage.gcsPath).isEqualTo(storageCloudStoragePath)
      assertThat(storage.toolResultsHistory.projectId).isEqualTo(storageHistoryName)
      assertThat(storage.toolResultsHistory.historyId).isEqualTo(storageHistoryId)
    }
