...
class TestMatrixRunProcessTrackerTest {
  ...
  @Mock lateinit var testRunMatrix: TestMatrix

  @Mock lateinit var mockResultStorage: ResultStorage
  ...
  fun getMatrixRunTracker(projectName: String = "project"): TestMatrixRunProcessTracker =
    TestMatrixRunProcessTracker(testing, projectName, 0L, logger)

  @Test
  fun test_waitForTestResults() {
    lateinit var expectedMatrix: TestMatrix

    `when`(testing.getTestMatrix("project", testRunMatrix))
      .thenReturn(
        createTestMatrix(
            "FINISHED",
            listOf(
  ...
  fun test_waitForTestResults_alwaysGivesDetails() {
    ...
    inOrder(logger).also {
      inOrder(logger).also {
        ...
        //                )
        //                it.verify(logger).lifecycle(
        //                    "Firebase Testlab Test for device: started results processing. Attempt
        // 1"
        //                )
        //                it.verify(logger).lifecycle(
  ...
  fun test_waitForTestResults_robust() {
    ...
    inOrder(logger).also {
      ...
      //            )
      //            it.verify(logger).lifecycle("Firebase Testlab Test for different: Preparing
      // device.")
      //            it.verify(logger).lifecycle(
      //                "Firebase Testlab Test for different: Retrieving Performance Environment
      ...
      //            )
      //            it.verify(logger).lifecycle(
      //                "Firebase Testlab Test for different: Starting results processing. Attempt:
      // 1"
      //            )
      //            it.verify(logger).lifecycle(
      //                "Firebase Testlab Test for different: Completed results processing. Time
      // taken = 5 (secs)"
      //            )
      it.verify(logger).info("Test execution: FINISHED")
      verifyNoMoreInteractions(logger)
