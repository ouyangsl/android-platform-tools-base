...
internal class ProcessNameMonitorFlowsImpl(
    ...
    override fun trackClients(device: IDevice): Flow<ClientMonitorEvent> =
        ClientsEventFlow(device, adbAdapter, logger).flow().flowOn(context)
}

/**
 * Creates a [Flow] of [ClientMonitorEvent]
 *
 * The 2 functions, handleClientListChanged and handleClientChanged, cannot be called concurrently
 * because they are only called from inside the same [FlowCollector] in
 * [ProcessNameMonitorFlowsImpl.trackClients]. This means that we do not need to worry about
 * concurrent access to the fields clients and preInitializedClients.
 *
 * Note that this class could be refactored completely into ProcessNameMonitorFlowsImpl.trackClients
 * but extracting here makes it more readable.
 */
private class ClientsEventFlow(
    ...
    suspend fun handleClientListChanged(
        flow: FlowCollector<ClientMonitorEvent>,
        newClients: MutableSet<Client>,
    ) {
        val addedClients = newClients - clients
        val addedProcessNames = addedClients.associateTo(hashMapOf()) {
            it.pid() to ProcessNames(it.applicationId(), it.processName())
        }
    ...
    private fun handlePreInitializedClients(
        processNames: MutableMap<Int, ProcessNames>,
        clients: MutableCollection<Client>,
    ) {
