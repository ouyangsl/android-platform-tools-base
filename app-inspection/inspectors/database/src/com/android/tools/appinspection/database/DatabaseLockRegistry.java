...
public class DatabaseLockRegistry {
    ...
    @GuardedBy("mLock")
    private final Map<Integer, Lock> mLockIdToLockMap = new HashMap<>();

    @GuardedBy("mLock")
    private final Map<Integer, Lock> mDatabaseIdToLockMap = new HashMap<>();

    @GuardedBy("mLock")
    private int mNextLockId = 1;
            ...
            Executors.newSingleThreadExecutor(
                    new ThreadFactory() {
                        ...
                        public Thread newThread(Runnable r) {
                            Thread thread =
                                    new Thread(r, "Studio:Sql:Lock"); // limit = 15 characters
     ...
     * provided multiple times (for lock requests on an already locked database), the lock needs to
     * be released by all previous requestors for the database to get unlocked.
     */
     ...
     */
    @Nullable
    DatabaseConnection getConnection(int databaseId) {
        synchronized (mLock) {
            Lock lock = mDatabaseIdToLockMap.get(databaseId);
            return (lock == null) ? null : new DatabaseConnection(lock.mDatabase, mExecutor);
        }
    }
                    ...
                    SqliteInspectionExecutors.submit(
                            ...
                            new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    database.rawQuery(
                                                    "ROLLBACK;", new String[0], cancellationSignal)
                                            .getCount(); // forces the cursor to execute the query
