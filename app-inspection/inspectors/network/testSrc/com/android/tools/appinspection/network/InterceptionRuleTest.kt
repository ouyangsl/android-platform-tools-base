...
class InterceptionRuleTest {
  ...
  fun addResponseHeader() {
    ...
    val addingNewHeaderAndValue =
      HeaderAdded.newBuilder()
        ...
        .build()
    var transformedResponse = HeaderAddedTransformation(addingNewHeaderAndValue).transform(response)
    ...
    val addingValueToExitingHeader =
      HeaderAdded.newBuilder()
        .apply {
          name = "Name"
          value = "Value2"
  ...
  fun replaceResponseHeader() {
    ...
    val multipleMatchedProto =
      valueNotMatchedProto
        .toBuilder()
        .apply {
          targetValueBuilder.apply {
            type = MatchingText.Type.REGEX
  ...
  fun replaceResponseHeaderPartially() {
    ...
    val headerNameReplacedProto =
      HeaderReplaced.newBuilder()
        ...
        .build()
    transformedResponse = HeaderReplacedTransformation(headerNameReplacedProto).transform(response)
    assertThat(transformedResponse.interception.headerReplaced).isTrue()
