...
// This class exists separately from `ConstantEvaluator` because
// the implementation can be much cleaner in Kotlin, but `ConstantEvaluator`
// needs to remain a Java file for its static methods to be
// backwards-compatible with old Kotlin call-sites.
internal class ConstantEvaluatorImpl(private val evaluator: ConstantEvaluator) {

  fun evaluate(node: UElement?): Any? =
    when {
      node == null -> null
      node is ULiteralExpression -> node.value
      node is UPrefixExpression ->
          ...
          when {
            ...
            operands.values.isEmpty() && node.sourcePsi is KtStringTemplateExpression -> ""
              ...
              when (node.operator) {
                ...
                UastBinaryOperator.PLUS -> operands.plus(evaluator.allowUnknown)
                  ...
                  when {
                    ...
                    selector.methodName == "trimMargin" ->
                      evaluate(receiver)?.tryOn { s: String ->
                        val prefix =
                          when (selector.valueArguments.size) {
  ...
  private fun <E> evalAsArray(elems: List<E>, elemType: PsiType, eval: (E) -> Any?): Any? =
    when {
      ...
      is PsiPolyadicExpression ->
        ArgList(node.operands.map(::evaluate)).let { operands ->
          when (node.operationTokenType) {
            ...
            JavaTokenType.PLUS -> operands.plus(evaluator.allowUnknown)
        ...
        when (val resolved = (node as PsiReference).resolve()) {
          is PsiField ->
            when (resolved.name) {
              ...
              "length" ->
                node
                  .tryOn(PsiReferenceExpression::getQualifierExpression)
        ...
        when (val type = node.type) {
          is PsiArrayType ->
            when (val initializer = node.arrayInitializer) {
              null ->
                node.arrayDimensions.firstOrNull()?.let(::evaluate)?.tryOn(Number::toInt)?.let {
                  size ->
  ...
  private fun surroundedByVariableCheck(node: UElement?, variable: PsiVariable): Boolean {
    ...
    tailrec fun check(curr: UIfExpression?): Boolean =
      when {
        curr == null -> false
  ...
  companion object {
    ...
    fun evaluateString(context: JavaContext?, node: PsiElement, allowUnknown: Boolean): String? =
      runEvaluator(allowUnknown, node, ConstantEvaluator::evaluate)
    ...
    tailrec fun isArrayLiteral(node: PsiElement?): Boolean =
      when (node) {
        is PsiReference ->
          when (val resolved = (node as PsiReference).resolve()) {
    ...
    /** Returns true if the node is pointing to an array literal */
    tailrec fun isArrayLiteral(node: UElement?): Boolean =
      when {
        node == null -> false
        node is UReferenceExpression ->
...
private value class ArgList<out X>(val values: List<X>) {
  inline fun <reified T0, reified T1, A : Any> tryOn(f: (T0, T1) -> A?): A? =
    when (values.size) {
...
private fun freshArray(type: PsiType, size: Int, dimensions: Int): Any =
  when {
