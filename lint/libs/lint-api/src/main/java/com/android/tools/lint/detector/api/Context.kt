    ...
    when (node) {
      ...
      is PsiElement -> {
        ...
        return when (type) {
          ...
          LocationType.CALL_WITH_ARGUMENTS,
          LocationType.CALL_WITH_RECEIVER -> context.getLocation(node)
          LocationType.VALUE -> error("$type not supported for ${node.javaClass}")
        }
      }
      is Node -> {
        if (client.isMergeManifestNode(node)) {
          ...
          if (source != null && source.second !== node) {
            ...
            val xmlContext = XmlContext(driver, project, main, source.first, null, null, doc)
        ...
        for (manifest in p.getManifestFiles()) {
          try {
            ...
            if (sourceNode != null && sourceNode !== element) {
              val doc = node.ownerDocument
              val xmlContext = XmlContext(driver, p, main, manifest, null, null, doc)
  ...
  companion object {
    ...
    private fun findPrefixOnPreviousLine(
      ...
      while (offset >= 0) {
        ...
        if (c == first && CharSequences.regionMatches(contents, offset, prefix, 0, prefix.length)) {
          return offset
        }
        offset--
      }
