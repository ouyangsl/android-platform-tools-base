...
private constructor(
  ...
  companion object Factory {
    ...
    private val ARTIFACT_PATTERN =
      Pattern.compile(
        ".*[/\\\\].gradle[/\\\\]caches[/\\\\]transforms-[0-9]+[/\\\\][0-9a-f]+[/\\\\]transformed[/\\\\](.+)[/\\\\]jars[/\\\\]lint\\.jar$"
      )
        ...
        val issues =
          try {
            ...
            if (reportErrors(driver)) {
              ...
              val message =
                "Lint found one or more custom checks that could not " +
          ...
          if (api < CURRENT_API) {
            ...
            try {
              ...
              if (minApi > CURRENT_API) {
                if (reportErrors(driver)) {
                  val message =
                    "Lint found an issue registry (`$className`) which " +
      ...
      return registry.vendor
        ?: run {
          ...
          val inferredVendor: Vendor? =
            if (registryClass.startsWith("androidx.") || registryClass.startsWith("android.")) {
              Vendor(
     ...
     */
    private fun loadAndCloseURLClassLoader(client: LintClient, file: File, loader: URLClassLoader) {
        ...
        JarFile(file).use { jar ->
          ...
          while (enumeration.hasMoreElements()) {
            ...
            if (path.endsWith(DOT_CLASS) && path.indexOf('$') == -1) {
              ...
              // to class name (.'s).
              val name = path.substring(0, path.length - DOT_CLASS.length).replace('/', '.')
              try {
                ...
                if (logJarProblems()) {
                  client.log(Severity.ERROR, e, "Failed to prefetch $name from $file")
                }
              }
            }
          }
        }
      } catch (ignore: Throwable) {} finally {
