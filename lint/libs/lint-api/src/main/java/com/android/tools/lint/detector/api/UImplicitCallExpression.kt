...
abstract class UImplicitCallExpression(
  ...
  override val uAnnotations: List<UAnnotation>
    @Suppress("ExternalAnnotations") get() = expression.uAnnotations
  ...
  override fun getArgumentForParameter(i: Int): UExpression? {
    ...
    // upstream [UCallExpression#getArgumentForParameter] returns `null` when an argument is not
    // found,
...
/** Visitor which visits all the calls in the target context -- including overloaded operators */
abstract class UastCallVisitor : AbstractUastVisitor() {
  abstract fun visitCall(node: UCallExpression): Boolean

  /** Don't override or call this method directly; instead, override [visitCall] */
