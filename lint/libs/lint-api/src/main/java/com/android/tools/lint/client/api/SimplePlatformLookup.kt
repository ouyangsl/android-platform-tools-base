...
internal class SimplePlatformLookup(private val sdkHome: File) : PlatformLookup {
  ...
  companion object {
    ...
    fun get(sdkHome: File): SimplePlatformLookup {
      return cache[sdkHome] ?: SimplePlatformLookup(sdkHome).also { cache[sdkHome] = it }
    }
      ...
      for (platformFolder in platformFolders) {
        ...
        if (
          ...
          if (packageXml.isFile) {
            platformFromPackageXml(platformFolder, packageXml)?.let { into.add(it) }
          } else {
            val sourceProperties = File(platformFolder, FN_SOURCE_PROP)
            if (sourceProperties.isFile) {
              platformFromSourceProp(platformFolder, sourceProperties)?.let { into.add(it) }
            }
          }
        }
      }
    ...
    private fun platformFromSourceProp(location: File, sourceProperties: File): IAndroidTarget? {
      try {
