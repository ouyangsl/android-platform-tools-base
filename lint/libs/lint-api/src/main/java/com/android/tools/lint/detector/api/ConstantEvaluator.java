...
public class ConstantEvaluator {
    ...
    public ConstantEvaluator(@Nullable JavaContext context) {
        ...
        this.uastContext = context != null ? context.getUastContext() : null;
    }

    public ConstantEvaluator(@Nullable JavaContext context, @NonNull UastContext uastContext) {
        this.context = context;
...
// TODO: The PSI version was fixed recently
                Number left = (Number) operandLeft;
                Number right = (Number) operandRight;
                boolean isInteger =
                        !(left instanceof Float || left instanceof Double
    ...
    @Nullable
    public static Object evaluate(@NonNull JavaContext context, @NonNull UElement element) {
        if (element instanceof ULiteralExpression) {
            return ((ULiteralExpression) element).getValue();
        }
        return evaluate(context, element, context.getUastContext());
    }
     ...
     * @param uastContext the UAST context
     * @return the corresponding constant value - a String, an Integer, a Float, and so on
     */
    @Nullable
    public static Object evaluate(@Nullable JavaContext context, @NonNull UElement element,
            @NonNull UastContext uastContext) {
        return new ConstantEvaluator(context, uastContext).evaluate(element);
    }
