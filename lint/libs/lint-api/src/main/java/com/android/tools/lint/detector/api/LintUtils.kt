...
fun startsWith(string: String, prefix: String, offset: Int): Boolean {
  return string.regionMatches(offset, prefix, 0, prefix.length, ignoreCase = true /* ignoreCase */)
}
 ...
 * @param createString If true, create a [String] instead of a general [CharSequence]
 * @return the string
 */
...
fun guessGradleLocationForFile(client: LintClient, gradle: File, string: String?): Location {
  if (gradle.isFile) {
    if (string == null) {
...
fun resolvePlaceHolder(project: Project?, name: String): String? {
...
fun isPolyadicFromStringTemplate(element: UElement?): Boolean {
  contract { returns(true) implies (element is UPolyadicExpression) }
    ...
    parameters.forEachIndexed { index, psiParameter ->
      call.getArgumentForParameter(index)?.let { argument ->
        if (argument is UExpressionList) { // vararg
          argument.expressions.forEach { mapping[it] = psiParameter }
        } else {
