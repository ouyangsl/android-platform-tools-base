        ...
        for (Detector detector : detectors) {
            ...
            if (names != null) {
                ...
                for (String name : names) {
                    List<VisitingDetector> list = mMethodDetectors
            ...
            if (applicableSuperClasses != null) {
                for (String fqn : applicableSuperClasses) {
                    List<VisitingDetector> list = mSuperClassDetectors
            ...
            if (referenceNames != null) {
                ...
                for (String name : referenceNames) {
                    List<VisitingDetector> list = mReferenceDetectors
                            .computeIfAbsent(name, k -> new ArrayList<>(SAME_TYPE_COUNT));
                    list.add(v);
                }
            }
            ...
            if (mVisitMethods) {
                ...
                if (methodName != null) {
                    ...
                    if (list != null) {
                        ...
                        if (function != null) {
                            UMethod method = mContext.getUastContext().getMethod(function);
                            for (VisitingDetector v : list) {
                                UastScanner scanner = v.getUastScanner();
                                if (scanner != null) {
                                    scanner.visitMethod(mContext, node, method);
                                }
                            }
                        }
                    }
        ...
        private void visitNewExpression(UCallExpression node) {
            if (mVisitConstructors) {
                ...
                if (resolvedClass != null) {
                    ...
                    if (list != null) {
                        UMethod method = mContext.getUastContext()
                                .getMethod(resolvedConstructor);
                        for (VisitingDetector v : list) {
                            UastScanner javaPsiScanner = v.getUastScanner();
                            if (javaPsiScanner != null) {
