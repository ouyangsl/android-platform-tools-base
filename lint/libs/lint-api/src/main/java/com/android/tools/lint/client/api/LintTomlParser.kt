...
interface LintTomlDocument {
     ...
     *
     * Note that the file contents may not match the document sources;
     * when running in the IDE for example, we'll be parsing the current
     * editor contents, not the most recently saved document. As always,
     * use [LintClient.readFile] to access file content; files are only
    ...
    /**
     * Looks up the corresponding value by a path; this is just a
     * convenience function for looking up using a flattened string.
     */
...
/**
 * Represents a value in a TOML value, which may or may not have an
 * associated key.
 */
interface LintTomlValue {
    ...
    /**
     * The associated key, if applicable. (This is only the local key in
     * the parent map, not the full key; see [getFullKey] for that.
     */
     ...
     *     delay = 1_000
     *
     * this will return java.lang.Integer(1000). Similarly, if called
     * on a String value, you get the actual String (without the
     * surrounding quotes, with escapes applied, etc.)
     *
     * If it cannot produce the correct value, it will return the source
     * text of the value instead. Note also that this method isn't
     * performing full TOML validation, so while for example TOML does
     * not allow leading zeroes in integers, lint will still return
     * the intended integer. Generally, the idea is for lint to try to
     * be able to run on partially broken source since the user may be
     * editing it, and we want to be able to offer guidance, sometimes
     * to help the user make the code valid.
     */
    ...
    /**
     * Returns the start offset of the value. Note that for a key=value
     * pair, this is for the value portion.
     */
    ...
    /**
     * Returns the start offset of the key for this value, or -1 if it
     * does not have a key.
     */
    fun getKeyStartOffset(): Int

    /**
     * Returns the end offset of the key for this value, or -1 if it
     * does not have a key.
     */
    ...
    /**
     * Returns the location of this value, including its key (if
     * applicable)
     */
    fun getFullLocation(): Location

    /**
     * *Optional* operation to return the fully qualified path of the
     * map value
     */
...
/**
 * Represents a map of values. The map iteration order matches the TOML
 * document.
 */
...
interface LintTomlLiteralValue : LintTomlValue {
    /**
     * Maps from a TOML source literal (such as `"foo"`) to the
     * corresponding value (such as new String("foo"))
     */
