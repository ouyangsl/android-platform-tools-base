...
public class SupportAnnotationDetector extends Detector implements UastScanner {
    ...
    private void checkContextAnnotations(@NonNull JavaContext context, @Nullable PsiMethod method,
        ...
        UElement p = skipParentheses(call.getContainingElement());

        if (p instanceof UQualifiedReferenceExpression) {
            call = p;
            p = p.getContainingElement();
        }
        ...
        for (UAnnotation annotation : annotations) {
            ...
            switch (signature) {
                ...
                case INT_DEF_ANNOTATION: {
                    boolean flag = getAnnotationBooleanValue(context, annotation, TYPE_DEF_FLAG_ATTRIBUTE)
                            == Boolean.TRUE;
        ...
        if (!requirement.isSatisfied(permissions)) {
            ...
            if (!requirement.isSatisfied(permissions)) {
                ...
                if (node instanceof UCallExpression) {
                    location = context.getCallLocation((UCallExpression) node, true, true);
                } else {
    ...
    private static String getIntRangeError(
        ...
        IntRangeConstraint constraint = IntRangeConstraint.create(context, annotation);
    ...
    private static void checkFloatRange(
        ...
        FloatRangeConstraint constraint = FloatRangeConstraint.create(context, annotation);
    ...
    private static void checkSize(
        ...
        SizeConstraint constraint = SizeConstraint.create(context, annotation);
    ...
    private class CallVisitor extends AbstractUastVisitor {
        ...
        @Override
        public boolean visitArrayAccessExpression(UArrayAccessExpression expression) {
