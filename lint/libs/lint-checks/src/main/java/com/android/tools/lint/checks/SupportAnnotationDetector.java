...
public class SupportAnnotationDetector extends Detector implements UastScanner {
        ...
        for (UAnnotation annotation : annotations) {
            ...
            if (COLOR_INT_ANNOTATION.equals(signature)) {
                ...
                if (signature.equals(INT_DEF_ANNOTATION)) {
                    boolean flag = getAnnotationBooleanValue(context, annotation, TYPE_DEF_FLAG_ATTRIBUTE) == Boolean.TRUE;
    ...
    private static void checkColor(@NonNull JavaContext context, @NonNull UElement argument) {
        ...
        EnumSet<ResourceType> types = ResourceEvaluator.getResourceTypes(context.getEvaluator(),
                context.getUastContext(),  argument);
        ...
        if (!requirement.isSatisfied(permissions)) {
            ...
            if (!requirement.isSatisfied(permissions)) {
                ...
                String message = getMissingPermissionMessage(requirement, name, permissions,
                        operation);
                context.report(MISSING_PERMISSION, node, context.getCallLocation(node, true, true), message);
            }
            ...
            if (!handlesMissingPermission && !isIgnoredInIde(MISSING_PERMISSION, context, node)) {
                ...
                Location location = context.getCallLocation(node, true, true);
                context.report(MISSING_PERMISSION, node, location, message);
            }
        }
    }
    ...
    private static void checkResult(@NonNull JavaContext context, @NonNull UCallExpression node,
        ...
        if (isExpressionValueUnused(node)) {
            ...
            String suggested = getAnnotationStringValue(context, annotation, ATTR_SUGGEST);
            ...
            context.report(issue, node, context.getCallLocation(node, true, true), message);
        }
    }
    ...
    public static int getVisibilityForTesting(@NonNull UAnnotation annotation) {
        UNamedExpression otherwise = annotation.findDeclaredAttributeValue(ATTR_OTHERWISE);
        if (otherwise != null) {
            UExpression value = otherwise.getExpression();
    ...
    private static void checkRestrictTo(
        ...
        if ((scope & RESTRICT_TO_TESTS )!= 0) {
            if (!isTestContext(bridge, node)) {
                reportRestriction(annotation, "from tests", containingClass, method, bridge,
                        node, isClassAnnotation);
            }
        }
    ...
    public static int getRestrictionScope(@NonNull UAnnotation annotation) {
        ...
        if (value != null) {
            return getRestrictionScope(value.getExpression());
        }
        return 0;
    }
    ...
    private static void checkThreading(
        ...
        if (threadContext != null && !isCompatibleThread(threadContext, signature)
            ...
            context.report(THREAD, node, context.getCallLocation(node, true, true), message);
        }
    }
        ...
        if (expression != null) {
            ...
            if (resolved instanceof PsiVariable) {
                ...
                UExpression lastAssignment =
                        UastLintUtils.findLastAssignment(variable, node, context.getUastContext());
    ...
    private static String getIntRangeError(
        ...
        long from = getLongAttribute(context, annotation, ATTR_FROM, Long.MIN_VALUE);
        long to = getLongAttribute(context, annotation, ATTR_TO, Long.MAX_VALUE);
    ...
    private static void checkFloatRange(
        ...
        double from = getDoubleAttribute(context, annotation, ATTR_FROM, Double.NEGATIVE_INFINITY);
        double to = getDoubleAttribute(context, annotation, ATTR_TO, Double.POSITIVE_INFINITY);
        boolean fromInclusive = getBoolean(context, annotation, ATTR_FROM_INCLUSIVE, true);
        boolean toInclusive = getBoolean(context, annotation, ATTR_TO_INCLUSIVE, true);
    ...
    private static void checkSize(
        ...
        long exact = getLongAttribute(context, annotation, ATTR_VALUE, -1);
        long min = getLongAttribute(context, annotation, ATTR_MIN, Long.MIN_VALUE);
        long max = getLongAttribute(context, annotation, ATTR_MAX, Long.MAX_VALUE);
        long multiple = getLongAttribute(context, annotation, ATTR_MULTIPLE, 1);
        ...
        if (argument instanceof ULiteralExpression) {
            ...
            if (resolved instanceof PsiVariable) {
                ...
                if (variable.hasModifierProperty(PsiModifier.STATIC)
                    ...
                    UExpression lastAssignment =
                            UastLintUtils.findLastAssignment(variable, argument, context.getUastContext());
    ...
    private class CallVisitor extends AbstractUastVisitor {
        ...
        private void checkCall(PsiMethod method, UCallExpression call) {
            ...
            if (!pkgAnnotations.isEmpty()) {
                for (UAnnotation annotation : pkgAnnotations) {
                    checkMethodAnnotation(mContext, method, call, annotation, methodAnnotations,
                            classAnnotations);
                }
            }
    ...
    private static class MyLintInspectionBridge extends LintInspectionBridge {
        ...
        public void report(
            ...
            context.report(issue, scopeNode, location, message);
        }
