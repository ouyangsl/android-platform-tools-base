...
/**
 * Reports all calls to setCommunicationDevice as issues, unless there is a call to
 ...
 *   issues. The source code of modules cannot be accessed at this stage.
 *
 * CommunicationDeviceDetector does the following: if there are no calls to clearCommunicationDevice
 * then report all calls to setCommunicationDevice as issues. In the analysis phase, the detector
 ...
 * issue. See [checkPartialResults], below.
 *
...
class CommunicationDeviceDetector : Detector(), SourceCodeScanner {
  ...
  /**
   * The number of calls to setCommunicationDevice() in the current lint invocation. This is used as
   * the next key for storing the location of a call to setCommunicationDevice in a LintMap. See
   * [visitMethodCall].
   *
   * A subtlety that does not matter for this detector, but could be confusing: In partial analysis
   * mode (in the analysis phase), there is a lint invocation (and so a new detector instance is
   * created) per module analyzed, so [numSetCalls] will conceptually reset to 0 for each module.
   * There is a LintMap per module and per [Issue], so keys will not collide. In global analysis
   * mode, lint will invoke the same detector instance on each module, so [numSetCalls] will not
   * reset to 0.
   */
