...
class FloatRangeConstraint extends RangeConstraint {
    ...
    @NonNull
    public static FloatRangeConstraint create(
            @NonNull JavaContext context,
            @NonNull UAnnotation annotation) {
        assert FLOAT_RANGE_ANNOTATION.equals(annotation.getQualifiedName());
        double from = getAnnotationDoubleValue(context, annotation, ATTR_FROM, Double.NEGATIVE_INFINITY);
        double to = getAnnotationDoubleValue(context, annotation, ATTR_TO, Double.POSITIVE_INFINITY);
        boolean fromInclusive = getAnnotationBooleanValue(context, annotation, ATTR_FROM_INCLUSIVE, true);
        boolean toInclusive = getAnnotationBooleanValue(context, annotation, ATTR_TO_INCLUSIVE, true);
        return new FloatRangeConstraint(from, to, fromInclusive, toInclusive);
    }
