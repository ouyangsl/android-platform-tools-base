...
/**
 * Searches for WebViews that can access any url By default, a WebView does open clicked links in
 * the browser However, if you set a WebViewClient the default behaviour changes to open all pages
 * within the WebView. The recommended way to overcome this is to override the
 * shouldOverrideUrlLoading This looks for setWebViewClient(WebViewClient client) calls where the
 * client class does not override the shouldOverrideUrlLoading method Note that his does not
 * actually check if the shouldOverrideUrlLoading is overwritten in a meaningful way. This check is
 * done by the WebViewClientMisuseDetector
 *
...
public class UnrestrictedWebViewDetector extends Detector implements Detector.UastScanner {
    ...
    public static final String MESSAGE =
            "The configured WebViewClient will open any page within your WebView "
    ...
    public static final Issue ISSUE =
            Issue.create(
                            ...
                            Severity.WARNING,
                            new Implementation(
                                    UnrestrictedWebViewDetector.class, Scope.JAVA_FILE_SCOPE))
                    .addMoreInfo(
                            "https://labs.mwrinfosecurity.com/blog/adventures-with-android-webviews/")
                    .addMoreInfo(
                            "https://developer.android.com/guide/webapps/managing-webview.html#safe-browsing");
    ...
    public void visitMethod(
        ...
        if (!evaluator.isMemberInSubClassOf(method, CLASS_WEB_VIEW, false)) return;
        List<UExpression> argumentValueList = call.getValueArguments();
        if (argumentValueList.size() != 1) return;
    ...
    private boolean isUnrestrictedWebViewClient(UExpression webViewClientArgument) {
        ...
        if (webViewClientClass == null) return false;
        // the default web view client class does allow to load any url
        if (CLASS_WEB_VIEW_CLIENT.equals(webViewClientClass.getQualifiedName())) return true;
        // a web view client that does not override "shouldOverrideUrlLoading" allows to load any url
        else if (webViewClientClass.findMethodsByName(SHOULD_OVERRIDE_URL_LOADING, false).length
                == 0) return true;
        return false;
    }
