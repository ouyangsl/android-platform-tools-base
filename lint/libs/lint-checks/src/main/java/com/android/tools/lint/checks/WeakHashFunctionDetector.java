...
/**
 * Checks for the use of weak hash functions. Currently, the only verified weak hash function is
 * MD5.
 *
 * <p>Insecure example: MessageDigest.getInstance("MD5");
 *
...
public class WeakHashFunctionDetector extends Detector implements UastScanner {
    ...
    public static final Issue ISSUE =
            Issue.create(
                    ...
                    LINT_PRIORITY,
                    LINT_SEVERITY,
                    new Implementation(WeakHashFunctionDetector.class, Scope.JAVA_FILE_SCOPE));
            ...
            public void visitCallExpression(UCallExpression uCallExpression) {
                ...
                for (UExpression ue : parameterList) {
                    ...
                    if (parameterValue != null
                            && parameterValue.toUpperCase().equals(L_WEAKHASHALGO)) {
                    ...
                    if (pe != null) {
                        ...
                        for (PsiElement parameter : resolvedParameters) {
                            ...
                            if (type.toString().equals("LITERAL_EXPRESSION")
                                    && parameter
                                            .getText()
                                            .toUpperCase()
                                            .equals("\"" + L_WEAKHASHALGO + "\"")) {
                                context.report(
                                        ISSUE, parameter, context.getLocation(parameter), LINT_MSG);
                                return;
                            }
                        }
                    }
