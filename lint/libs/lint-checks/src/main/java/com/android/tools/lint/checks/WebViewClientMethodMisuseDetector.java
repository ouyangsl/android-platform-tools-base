 ...
 *
 * <p>The current checks extend the inner class WebViewClientMethodVisitor.
 *
...
public class WebViewClientMethodMisuseDetector extends Detector implements Detector.UastScanner {
    ...
    public abstract class WebViewClientMethodVisitor extends AbstractUastVisitor {
        ...
        public void afterVisitMethod(@NonNull UMethod method) {
            ...
            if (!isMethodAcceptable()) report(method);
        }
    }

    public static final Issue UNRESTRICTED_INTERCEPT_REQUEST =
            Issue.create(
                    "SlackWebViewClient", //$NON-NLS-1$
                    ...
                    Severity.WARNING,
                    new Implementation(
                            WebViewClientMethodMisuseDetector.class, Scope.JAVA_FILE_SCOPE));
    ...
    public class ShouldInterceptRequestVisitor extends WebViewClientMethodVisitor {
        ...
        protected Issue getIssue() {
            return UNRESTRICTED_INTERCEPT_REQUEST;
        }

        @VisibleForTesting
        public static final String MESSAGE =
                "shouldInterceptRequest which allows any content to be loaded";
    ...
    public static final Issue UNRESTRICTED_OVERRIDE_URL_LOADING =
            Issue.create(
                    "SlackWebViewClient", //$NON-NLS-1$
    ...
    /**
     * Verification of shouldOverrideUrlLoading if it will not return false, or if it calls
     * view.loadUrl() on the given URL before returning true.
     */
    @VisibleForTesting
    public class ShouldOverrideUrlLoadingVisitor extends WebViewClientMethodVisitor {
        @Override
        protected Issue getIssue() {
            return UNRESTRICTED_OVERRIDE_URL_LOADING;
        }

        @VisibleForTesting
        public static final String MESSAGE =
                "shouldOverrideUrlLoading which always returns false or always loads the url";
         ...
         *
         * <p>For example, it checks if view.loadUrl(url); exists within the method
         * shouldOverrideUrlLoading.
         *
         * <p>The UAST hierarchy for such calls is structured as: 0: UCallExpression 1:
         * UQualifiedReferenceExpression 2: UBlockExpression 3: UMethod
         *
         * <p>Therefore we need to check that the third parent is the method to ensure that it is an
         * unconditional call.
         *
         * @param methodInvocation the call expression for which we want to know if it is
         *     unconditional
    ...
    public static final Issue PROCEED_ON_SSL_ERROR =
            Issue.create(
                            "ProceedOnSslError", //$NON-NLS-1$
                            ...
                            new Implementation(
                                    WebViewClientMethodMisuseDetector.class, Scope.JAVA_FILE_SCOPE))
                    .addMoreInfo(
                            "https://bitbucket.org/secure-it-i/android-app-vulnerability-benchmarks/src/978fbc17a087/Web/WebViewIgnoreSSLWarning-MITM-Lean/?at=master");

    /**
     * Checks that onReceivedSslError does not call handler.proceed(). Instead, handler.cancel()
     * should be called. We also assume that handler.cancel() is called if the handler is passed to
     * any further methods.
     */
    @VisibleForTesting
    public class OnReceivedSslErrorVisitor extends WebViewClientMethodVisitor {
        @Override
        protected Issue getIssue() {
            return PROCEED_ON_SSL_ERROR;
        }

        @VisibleForTesting
        public static final String MESSAGE = "onReceivedSslError which always proceeds";
        ...
        public boolean visitCallExpression(@NonNull UCallExpression methodInvocation) {
            ...
            if (receivesHandlerAsArgument(methodInvocation)) foundMethodReceivingHandler = true;
            return super.visitElement(methodInvocation);
        }

        private static final String SSL_ERROR_HANDLER = "android.webkit.SslErrorHandler";
        private static final String HANDLER_CANCEL = "cancel";
        private static final String HANDLER_PROCEED = "proceed";
