...
class AlarmDetector : Detector(), SourceCodeScanner, XmlScanner {
    companion object Issues {
        ...
        @JvmField
        val SCHEDULE_EXACT_ALARM = Issue.create(
        ...
        // Used as a partial results map key to note that `AlarmManager#canScheduleExactAlarms` is called somewhere
    ...
    // Same strategy as in CommunicationDeviceDetector - we keep track of the locations of all calls to
    ...
    override fun checkPartialResults(context: Context, partialResults: PartialResult) {
        ...
        for (perModuleLintMap in partialResults.maps()) {
            perModuleLintMap.forEach {
                // There should only be locations in the map since no map contained `CHECKS_EXACT_ALARM_PERMISSION`
