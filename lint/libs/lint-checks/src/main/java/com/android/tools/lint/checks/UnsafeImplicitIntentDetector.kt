...
class UnsafeImplicitIntentDetector : Detector(), SourceCodeScanner, XmlScanner {
    ...
    private fun isIntentImplicit(node: UCallExpression, context: JavaContext): Triple<Boolean, String?, Location?> {
    ...
    private fun buildQuickFix(context: Context, location: Location, longClassName: String): LintFix {
        ...
        val setPackage = LintFix.create()
            ...
            .build()
        val setClassName = LintFix.create()
            .name("Set class name")
            .replace()
            .reformat(true)
            .range(location)
    ...
    private fun getPackageNameAndClassName(context: Context, longClassName: String): Pair<String, String> {
