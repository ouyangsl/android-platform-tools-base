...
/**
 * This detector checks if an implicit intent is created and sent without any additional protection
 * through a permission specified in the sending method. Important: This detector considers only
 * intra-procedural issues. Intents are implicit as long as their receiver is not defined
 * specifically. Receivers can be defined specifically by the component (Class or ComponentName)
 * during intent creation or later. We only consider an implicit intent to be safe if the sending
 * method (like sendBroadcast) also receives a permission argument.
 *
 ...
 */
public class ImplicitUnprotectedIntentSendDetector extends Detector
    ...
    public static final String IMPLICIT_INTENT_MESSAGE =
            "Avoid sending implicit intents if possible";
    public static final Issue IMPLICIT_INTENT_SENDED_UNPROTECTED =
            Issue.create(
                            "UnauthorizedIntent", //$NON-NLS-1$
    ...
    public static final String IMPLICIT_PENDING_INTENT_MESSAGE =
            "Do not use implicit intents for pending intents";
    public static final Issue IMPLICIT_PENDING_INTENT =
            Issue.create(
                            "ImplicitPendingIntent", //$NON-NLS-1$
                    ...
                    .addMoreInfo(
                            "https://bitbucket.org/secure-it-i/android-app-vulnerability-benchmarks/src/978fbc17a087d77b38474bfbe01a00b5e2217ce6/ICC/ImplicitPendingIntent-IntentHijack-Lean/?at=master")
                    .addMoreInfo(
    ...
    public void visitMethod(
        ...
        if (!evaluator.isMemberInSubClassOf(method, CONTEXT_CLASS, false)
                && !evaluator.isMemberInSubClassOf(method, ACTIVITY_CLASS, false)
                && !evaluator.isMemberInSubClassOf(method, PENDING_INTENT_CLASS, false)) return;

        // Calls which include a non null permission are ok
        if (checkCallIncludesPermission(call)) return;

        UExpression intentArgument = getIntentArgument(call);
        // calls to these methods without intent argument aren't interesting
        if (intentArgument == null) return;
        ...
        if (lastAssignment == null
                || !UastExpressionUtils.isConstructorCall(lastAssignment)
                || isExplicitIntentConstructor(lastAssignment)) return;
        ...
        if (intentArgument instanceof USimpleNameReferenceExpression) {
            ...
            if (e instanceof PsiVariable) {
                ...
                if (!visitor.intentWasMadeExplicit()) report(call, isPendingIntent, context);
            }
        }
    }
    ...
    private boolean isExplicitIntentConstructor(@Nullable UExpression expression) {
        ...
        if (INTENT_CLASS.equals(klass)) {
            ...
            if (valueArgumentList.size() != 2 && valueArgumentList.size() != 4) return false;
            // for all implicit constructors with 2 or 4 arguments, the last argument is a uri
            PsiType lastType =
                    valueArgumentList.get(valueArgumentList.size() - 1).getExpressionType();
    ...
    private UExpression getIntentArgument(@NonNull UCallExpression call) {
        ...
        for (UExpression intentArgument : argumentValueList) {
            ...
            if (hasClassOrSuperClass(argumentType, INTENT_CLASS)) return intentArgument;
        }
        return null;
    }
    ...
    private void report(
            @NonNull UCallExpression call, boolean isPendingIntent, @NonNull JavaContext context) {
        if (isPendingIntent)
            context.report(
                    IMPLICIT_PENDING_INTENT,
                    call,
                    context.getLocation(call),
                    IMPLICIT_PENDING_INTENT_MESSAGE);
        else
            context.report(
                    IMPLICIT_INTENT_SENDED_UNPROTECTED,
                    call,
                    context.getLocation(call),
                    IMPLICIT_INTENT_MESSAGE);
    }
    ...
    private boolean isPendingIntent(@NonNull UCallExpression call) {
        if (call.getMethodName() == null) return false;
        switch (call.getMethodName()) {
            case "getActivity":
            case "getBroadcast":
            case "getService":
    ...
    private class SendIntentMethodVisitor extends AbstractUastVisitor {
        ...
        private boolean isComponentSettingMethod(@NonNull UCallExpression call) {
            for (String componentSettingMethods : componentSettingMethods()) {
                if (methodHasName(call, componentSettingMethods)) return true;
            }
            return false;
        }
        ...
        private List<String> componentSettingMethods() {
            return Arrays.asList(
                    "setClass", "setClassName", "setComponentName", "setPackage", "setComponent");
        }
