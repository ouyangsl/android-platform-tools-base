...
/**
 * This detector searches for run time registrations of receivers that do not consider any
 * permissions.
 *
 * <p>Example: Context.registerReceiver(broadcastReceiver, new
 * IntentFilter("package.action.SOME_ACTION"));
 *
 ...
 */
public class UnsafeDynamicBroadcastReceiverDetector extends Detector
        implements Detector.UastScanner {

    private static final String CONTEXT_CLASS = "android.content.Context";

    @VisibleForTesting
    public static final String NO_PERMISSION_ARGUMENT_MESSAGE =
            "Missing grant revokings can leave an app vulnerable to intent spoofing";

    @VisibleForTesting
    public static final String EMPTY_PERMISSION_ARGUMENT_MESSAGE =
            "Null objects should not be used as permission arguments";

    public static final Issue ISSUE =
            Issue.create(
                    "UnprotectedBroadcastReceiver", //$NON-NLS-1$
                    ...
                    Severity.WARNING,
                    new Implementation(
                            UnsafeDynamicBroadcastReceiverDetector.class, Scope.JAVA_FILE_SCOPE));
    ...
    public void visitMethod(
        ...
        if (!registersNonNullReceiver(argumentValueList)) return;
        ...
        if (!nonEmtpyPermissionArgument(argumentValueList)) {
            context.report(
                    ISSUE, call, context.getLocation(call), EMPTY_PERMISSION_ARGUMENT_MESSAGE);
        }
    }

    /**
     * Checks if the call signature used for registerReceiver contains a permission argument, i.e.,
     * this returns true if one of the following signatures was used:
     * registerReceiver(BroadcastReceiver receiver, IntentFilter filter, String broadcastPermission,
     * Handler scheduler, int flags) registerReceiver(BroadcastReceiver receiver, IntentFilter
     * filter, String broadcastPermission, Handler scheduler)
     *
     * @param argumentCount the number of arguments the registerReceiver method call receives
     * @return true if the number of arguments is 4 or 5 and thus we know the call contains an
    ...
    /**
     * Checks if the argument "String broadcastPermission" of the call to registerReceiver is not
     * null
     *
    ...
    /**
     * Checks if the argument "BroadcastReceiver receiver" of the call to registerReceiver is not
     * null. In the Android framework a null value stands for "no receiver is registered", and only
     * active sticky broadcasts will be returned.
     *
     * @param argumentValueList the arguments list of the registerReceiver methods invocation
