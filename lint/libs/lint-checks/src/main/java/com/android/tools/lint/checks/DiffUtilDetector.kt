...
class DiffUtilDetector : Detector(), SourceCodeScanner {
  ...
  private fun defaultEquals(context: JavaContext, type: PsiClassType?): Boolean {
    ...
    for (m in cls.findMethodsByName("equals", true)) {
      if (m is PsiMethod) {
        ...
        if (parameters.size == 1 && parameters[0].type.canonicalText == JAVA_LANG_OBJECT) {
  ...
  companion object {
    private val IMPLEMENTATION = Implementation(DiffUtilDetector::class.java, Scope.JAVA_FILE_SCOPE)
