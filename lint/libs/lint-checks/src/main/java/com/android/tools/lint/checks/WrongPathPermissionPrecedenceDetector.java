 ...
 * protected by a permission. The problem is the path permission that should not be used to further
 * restrict access to specific paths of a permission protected content provider.
 *
 * <p>Note: This detector is not aware of data sensitivity, i.e., it does not know if a permission
 ...
 * "search_suggest_query". That path permission is commonly used to enable suggestions in Android's
 * search bars and the corresponding data should not contain any sensitive information as it is
 * publicly available per definition.
 *
...
public class WrongPathPermissionPrecedenceDetector extends Detector implements Detector.XmlScanner {
    ...
    public void visitElement(@NonNull XmlContext context, @NonNull Element element) {
        if (!SecurityDetector.getExported(element) || !hasPermission(element)) return;
        ...
        for (Element pathPermissionChild : LintUtils.getChildren(element)) {
            ...
            if (TAG_PATH_PERMISSION.equals(pathPermissionChild.getNodeName())) {
                /**
                 * Ignore path permission for search suggest query paths. These path permissions are
                 * unproblematic, since they intentionally release non-sensitive data.
                 */
                if (protectsASearchSuggestPath(pathPermissionChild)) return;
                context.report(
                        ISSUE,
                        pathPermissionChild,
                        context.getLocation(pathPermissionChild),
                        MESSAGE);
            }
        }
    }
    ...
    private static boolean protectsASearchSuggestPath(Element element) {
        String[] pathAttributeValues = {
            element.getAttributeNS(ANDROID_URI, ATTR_PATH),
        ...
        for (String pathAttribute : pathAttributeValues) {
            if (pathAttribute != null
                    && pathAttribute.matches(".*" + SEARCH_SUGGEST_QUERY + ".*")) {
                return true;
            }
        }
        return false;
    ...
    private static boolean hasPermission(Element element) {
        ...
        String[] permissionAttributeValues = {
            element.getAttributeNS(ANDROID_URI, ATTR_PERMISSION),
