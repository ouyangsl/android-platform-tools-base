 ...
 */
public class PathPermissionProblematicUriMatchingDetector extends Detector
    ...
    public void visitConstructor(JavaContext context, UCallExpression node, PsiMethod constructor) {
        ...
        if (providerXmlElement == null) return;
        if (manifestContext != null)
            manifestContext.report(
                    ISSUE,
                    providerXmlElement,
                    manifestContext.getLocation(providerXmlElement),
                    MESSAGE);
    }
    ...
    public void visitElement(@NonNull XmlContext context, @NonNull Element element) {
        ...
        if (!SecurityDetector.getExported(element)) return;
        ...
        for (Element child : LintUtils.getChildren(element)) {
            if (TAG_PATH_PERMISSION.equals(child.getNodeName())) {
                ...
                String unqualifiedProviderClassName =
                        providerClassNameParts[providerClassNameParts.length - 1];
