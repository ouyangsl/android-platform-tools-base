...
public class ApiDetector extends ResourceXmlDetector
    ...
    private final class ApiVisitor extends AbstractUastVisitor {
        ...
        @Override
        public boolean visitImportStatement(@NonNull UImportStatement statement) {
        ...
        // TODO: Temporary workaround for the fact that we don't have a separate visitor
        // call for method references (UCallableReferenceExpression looks relevant, but
        // isn't a UResolvable, and method references aren't currently converted into
        // UCallableReferenceExpression by the UAST converter; instead, these are simply
        // left as JavaUQualifiedReferenceExpressions but backed by
        // PsiMethodReferenceExpression which is how we work around it for now.
        @Override
        public boolean visitQualifiedReferenceExpression(UQualifiedReferenceExpression node) {
            if (node instanceof PsiElementBacked) {
                PsiElement psi = ((PsiElementBacked) node).getPsi();
                if (psi instanceof PsiMethodReferenceExpression) {
                    PsiElement resolved = ((PsiMethodReferenceExpression) psi).resolve();
        ...
        @Override
        public boolean visitBinaryExpressionWithType(@NonNull UBinaryExpressionWithType node) {
        ...
        public void checkMethod(@NonNull UMethod method) {
        ...
        private void checkClass(UClass aClass) {
        ...
        @Override
        public boolean visitClassLiteralExpression(UClassLiteralExpression expression) {
            if (willScanBytecode) {
                return true;
            }
        ...
        private void checkForeach(UForEachExpression statement) {
            if (willScanBytecode) {
                return;
            }
        ...
        private void checkCall(@NonNull UCallExpression expression) {
                ...
                if (REQUIRES_API_ANNOTATION.equals(annotation.getQualifiedName())) {
                    ...
                    if (api > minSdk) {
                        ...
                        if (target == -1 || api > target) {
                            if (isWithinVersionCheckConditional(mContext.getUastContext(),
                                    expression, api)) {
                                return true;
                            }
                            if (isPrecededByVersionCheckExit(mContext.getUastContext(),
                                    expression, api)) {
                                return true;
                            }
        ...
        @Override
        public boolean visitVariable(@NonNull UVariable variable) {
        ...
        @Override
        public boolean visitBinaryExpression(@NonNull UBinaryExpression expression) {
        ...
        public boolean visitTryExpression(UTryExpression statement) {
            ...
            if (resourceList != null && !resourceList.isEmpty()) {
                ...
                if (api > minSdk && api > getTargetApi(statement)) {
                    ...
                    PsiResourceListElement first = resourceList.get(0);
                    PsiResourceListElement last = resourceList.get(resourceList.size() - 1);
        ...
        @Override
        public boolean visitSwitchExpression(@NonNull USwitchExpression statement) {
            checkSwitch(statement);
            return true;
        }

        private void checkSwitch(@NonNull USwitchExpression statement) {
            if (willScanBytecode) {
                return;
            }
        ...
        private boolean checkField(@NonNull UElement node, @NonNull PsiField field) {
            ...
            if (api != -1) {
                ...
                if (api > minSdk
                    ...
                    //noinspection ConstantConditions
                    while (locationNode.getContainingElement() instanceof UQualifiedReferenceExpression
                            ...
                            && ((UQualifiedReferenceExpression)locationNode.getContainingElement()).getSelector() == locationNode) {
                        locationNode = locationNode.getContainingElement();
                    }
    ...
    private static boolean isSuppressed(
        ...
        return driver.isSuppressed(context, UNSUPPORTED, element)
                ...
                || isWithinVersionCheckConditional(context.getUastContext(), element, api)
                || isPrecededByVersionCheckExit(context.getUastContext(), element, api);
