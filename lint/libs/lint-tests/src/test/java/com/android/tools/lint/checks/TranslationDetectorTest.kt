...
class TranslationDetectorTest : AbstractCheckTest() {
  ...
  fun testMissingPluralTranslationBatch() {
    ...
    lint()
      .files(valuesStringsPlural, valuesFrStringsNoPlural)
      .run()
      .expect(
        """
  ...
  fun testBcp47() {
    val expected =
      """
            ...
            1 errors, 0 warnings
            """
    lint().files(valuesStrings, valuesTlhStrings).run().expect(expected)
  }
  ...
  fun testSpecifiedLanguageOk() {
    lint().files(strings13, valuesEsStrings, valuesEsUsStrings).run().expectClean()
  }
  ...
  fun testMissingSomeBaseStrings() {
    val expected =
      """
            ...
            5 errors, 0 warnings
            """
    lint().files(strings14, valuesEsStrings).run().expect(expected)
  }
  ...
  fun testExtraResourcesOfOtherTypesIncremental2() {
    lint()
      ...
      .incremental("res/values-v21/dimen.xml")
      .run()
      .expectClean()
  }
