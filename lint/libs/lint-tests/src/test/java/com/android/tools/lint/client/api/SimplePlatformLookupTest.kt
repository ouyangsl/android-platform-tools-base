...
class SimplePlatformLookupTest {
  ...
  @get:Rule val homeFolder = TemporaryFolder()
  ...
  private fun checkWithSimple(sdkFolder: File, checks: (PlatformLookup) -> Unit) {
  ...
  @Test
  fun testNotExistent() {
    assertEquals("[]", createSimpleLookup(File("/non/exist/ent")).getTargets().toString())
  }
  ...
  fun testEmpty() {
    ...
    assertEquals("[]", createSimpleLookup(root).getTargets().toString())
  }
  ...
  fun testLookupVersion() {
    checkQueries { lookup ->
      assertEquals("null", lookup.getTarget("unknown").describe())
  ...
  fun testCompareRealAndroidSdk() {
    ...
    val home = System.getenv("ANDROID_SDK_ROOT") ?: System.getenv("ANDROID_HOME") ?: return
  ...
  private fun createSampleAddOn(sdk: File, vendor: String, id: String, api: Int) {
