...
class CustomRuleTest {
  ...
  fun testProjectLintJarWithServiceRegistry() {
    ...
    lint()
      ...
      .customRules(lintJarWithServiceRegistry)
      .allowMissingSdk()
      .run()
      .expect(expected)
  }
  ...
  fun testOlderLintApiOk() {
    lint()
      .files(manifest().minSdk(1), java("package test.pkg;\n" + "public class Test {\n" + "}"))
  ...
  fun testLintScannerXmlAll() {
    ...
    lint()
      ...
      .check({
        assertThat(it)
          .contains(
            "res/values/strings.xml:2: Warning: All tags are now flagged: string [ShortUniqueId from com.example.lint.checks.sample]"
          )
      })
  }
  ...
  private val appCompatTestSource =
    java(
      "" +
        "package test.pkg;\n" +
        "\n" +
  ...
  private val appCompatTestClass =
    base64gzip(
