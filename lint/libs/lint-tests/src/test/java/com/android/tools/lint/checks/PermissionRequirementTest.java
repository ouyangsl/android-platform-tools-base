...
public class PermissionRequirementTest extends TestCase {
    ...
    private static UAnnotation createUAnnotation(
        ...
        List<UNamedExpression> expressions = Lists.newArrayList();
        when(annotation.getAttributeValues()).thenReturn(expressions);
        ...
        for (MockValue value : values) {
            ...
            UNamedExpression named = new UNamedExpression(name, null);
            named.setExpression(literal);

            when(annotation.findAttributeValue(value.name)).thenReturn(named);
            when(annotation.findDeclaredAttributeValue(value.name)).thenReturn(named);
