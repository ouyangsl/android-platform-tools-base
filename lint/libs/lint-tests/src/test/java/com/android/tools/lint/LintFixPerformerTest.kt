...
class LintFixPerformerTest : TestCase() {
  ...
  fun check(
    ...
    val performer =
      object : LintFixPerformer(client, printStatistics, includeMarkers = includeMarkers) {
  ...
  fun testXmlSetAttribute() {
    ...
    val fix =
      fix()
        .set(SdkConstants.ANDROID_URI, "new_attribute", "new value")
        .range(range)
        .autoFix()
        .build()
    check(
  ...
  fun testXmlSetAttributeOrder1() {
    ...
    val fix =
      fix()
        .set(SdkConstants.ANDROID_URI, "layout_height", "wrap_content")
        .range(range)
        .autoFix()
        .build()
    check(
  ...
  fun testXmlSetAttributeOrder2() {
    ...
    val range = Location.create(file, source, source.indexOf("element1"), source.length)
    val fix = fix().set(SdkConstants.ANDROID_URI, "id", "@+id/my_id").range(range).autoFix().build()
    check(
      file,
      source,
      fix,
  ...
  fun testXmlSetAttributeOrder3() {
    ...
    val range = Location.create(file, source, source.indexOf("element1"), source.length)
    val fix = fix().set(SdkConstants.ANDROID_URI, "z-order", "5").range(range).autoFix().build()
    check(
      file,
      source,
      fix,
  ...
  fun testXmlComposite() {
    ...
    val setFix =
      fix().set(SdkConstants.ANDROID_URI, "new_attribute", "new value").range(range).build()
  ...
  fun testAttributeSorting() {
    ...
    val sorted =
      attributes.sortedWith(Comparator { a, b -> LintFixPerformer.compareAttributeNames(a, b) })
    assertEquals(
    ...
    check(
      ...
      expected =
        """
            ...
            """,
      expectedOutput =
        "Applied 1 edits across 1 files for this fix: Replace with (android.graphics.drawable.Drawable drawable, android.graphics.Outline outline)",
      includeMarkers = true
    )
  }
}
