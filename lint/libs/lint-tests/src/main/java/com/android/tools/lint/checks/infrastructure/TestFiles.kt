...
object TestFiles {
  ...
  @JvmStatic
  fun binaryStub(into: String, vararg source: TestFile, byteOnly: Boolean = true): TestFile {
  ...
  @JvmStatic
  fun compiled(into: String, source: TestFile, vararg encoded: String): TestFile {
  ...
  @JvmStatic
  fun compiled(into: String, source: TestFile, checksum: Long, vararg encoded: String): TestFile {
    val type = getCompileType(BytecodeTestFile.Type.SOURCE_AND_BYTECODE, source)
  ...
  @Deprecated("") // Use the method with the checksum instead
  @JvmStatic
  fun bytecode(into: String, source: TestFile, vararg encoded: String): TestFile {
    val type = getCompileType(BytecodeTestFile.Type.BYTECODE_ONLY, source)
    return CompiledSourceFile(into, type, source, null, encoded)
  }

  @JvmStatic
  fun bytecode(into: String, source: TestFile, checksum: Long, vararg encoded: String): TestFile {
    val type = getCompileType(BytecodeTestFile.Type.BYTECODE_ONLY, source)
    return CompiledSourceFile(into, type, source, checksum, encoded)
  }
