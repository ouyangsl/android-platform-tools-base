 ...
 */
class FullyQualifyNamesTestMode :
  ...
  override fun transform(
    ...
    root.acceptSourceFile(
      object : TypeVisitor(context, source) {
  ...
  abstract class TypeVisitor(private val context: JavaContext, private val source: String) :
    ...
    private fun replaceClassReference(cls: PsiClass, node: UElement, type: PsiType?) {
      ...
      if (psi.containingFile is PsiCompiledFile) {
        ...
        // which will fail with an assertion error that complains Document, the mirrored text, is
        // `null`.
        return
      }
      if (
      ...
      // the symbol to match the current symbol, unless it matches an import alias or constructor
      // delegation
      if (!source.regionMatches(start, fqn, nameStart, nameLength)) {
        if (
          context.uastFile!!.imports.none {
    ...
    override fun visitMethod(node: UMethod): Boolean {
      ...
      node.returnTypeReference?.let { checkTypeReference(it) }
    ...
    /** Returns true if there's a variable in a surrounding scope of the given name */
    ...
    override fun visitBinaryExpressionWithType(node: UBinaryExpressionWithType): Boolean {
      node.typeReference?.let { checkTypeReference(it) }
