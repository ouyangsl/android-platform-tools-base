...
/**
 * Special test file which given a series of stub files can generate a
 * .jar file with compiled class files corresponding to the stubs.
 *
 * Note that this only works for stub files, not files with non-trivial
 * method bodies. The main complications this class deal with are
 * generics signatures, annotations in APIs and field initializers.
 *
 * It only works at the PSI class file level; it does not generate
 * Kotlin augmented metadata files. Therefore, you can include top level
 * functions, companion objects etc, and the stubs will look as they do
 * from Java code.
 *
 * TODO: Use the kotlin metadata library to also construct binary
 *     content for things like package level methods; see
...
internal open class StubClassFile(
    ...
    /**
     * Any library-only (needed for compilation, but not to be packaged)
     * dependencies
     */
    ...
    override fun getBytecodeFiles(): List<TestFile> {
        return bytecodeFiles
            ?: createClassFiles().also { bytecodeFiles = it }
    }
        ...
        try {
            ...
            try {
                ...
                if (SystemInfo.isWindows) {
                    // Sometimes the runtime environment is still hanging on to .jar files and Windows file locking blocks
    ...
    private fun getByteCode(context: JavaContext, file: UFile): List<TestFile> {
        ...
        file.accept(object : AbstractUastVisitor() {
            ...
            override fun visitClass(node: UClass): Boolean {
                ...
                //    "java jdk.internal.org.objectweb.asm.util.ASMifier com/example/myapplication/DiffUtil\$ItemCallback.class"
                ...
                if (internalName != null) {
                    ...
                    val hasSyntheticConstructorArgument0: Boolean = if (containingClass != null) {
                        val outerClassFieldVisitor = cw.visitField(
                            ACC_FINAL + ACC_SYNTHETIC,
                            "this$1", "L${containingClass.internalName()};", null, null
                        )
                    ...
                    for (method in node.uastDeclarations.filterIsInstance<UMethod>()) {
                        ...
                        val methodSignature = getGenericsSignature(
                            psiMethod,
                    ...
                    for (field in cls.fields) {
                        ...
                        val fv = cw.visitField(
                            getModifiers(field), field.name, descriptor, fieldSignature,
    ...
    private fun writeAttribute(name: String?, value: PsiElement?, visitor: AnnotationVisitor, type: PsiType?) {
        if (value is PsiReference) {
            ...
            if (name == null) {
                // Already created array in caller (where PSI has a single element but bytecode should have array to match property)
            ...
            // Handle some basic expressions in the stub sources too, such as `static final int constant = "test".length();`
            val constant = ConstantEvaluator.evaluate(null, value as PsiElement)
                ?: error("Annotation attribute type not yet supported: $value")
    ...
    companion object {
        /**
         * Is this method a simple method which the stub interpreter can
         * handle? (e.g. returns of constants or throws)
         */
        ...
        // See https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-4.html -- section 4.3.4 Signatures
                    ...
                    for (extendsType in extendsTypes) {
                        ...
                        if (first) {
                            if (extendsType.resolve()?.isInterface == true) {
                                // This is weird, but I see "T::" for type variables where there isn't a super class, only super
                                // interface
                                signature.append(':')
                            }
                            first = false
            ...
            if (resolved != null) {
                if (resolved is PsiTypeParameter) {
                    ...
                    if (outerMost == resolved) {
                        ...
                        val canonical = outerQualified.replace('.', '/') +
