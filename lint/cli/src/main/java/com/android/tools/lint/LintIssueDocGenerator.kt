...
class LintIssueDocGenerator constructor(
    ...
    private fun writeSuppressInfo(
        ...
        val language =
            issueData?.suppressExample?.files?.firstOrNull { containsSuppress(it.source, issue) }?.language
    ...
    private fun findSourceFiles(prefixMap: Map<String, List<File>>): Map<String, Map<File, List<File>>> {
        ...
        for ((prefix, path) in prefixMap) {
            for (dir in path) {
                ...
                val dirMap = map[prefix] ?: mutableMapOf<File, MutableList<File>>().also { map[prefix] = it }
    ...
    private fun initializeExamples(issueData: IssueData) {
        ...
        val virtualFile = local.findFileByPath(source.path) ?: error("Could not find virtual file for $source")
        ...
        val suppressExampleNames = setOf("testSuppressExample", "testSuppressExample$id", "suppressExample")
    ...
    private fun computeResultMap(
        ...
        while (true) {
            ...
            if (matcher.find()) {
                ...
                if (!MESSAGE_PATTERN.matcher(sourceLine1).matches()) { // else: no source included in error output
    ...
    companion object {
        ...
        private fun getRegistry(client: LintCliClient, jars: List<File>, includeBuiltins: Boolean): IssueRegistry? {
            if (jars.isEmpty()) {
        ...
        fun run(args: Array<String>, fromLint: Boolean = false): Int {
            ...
            val generator = LintIssueDocGenerator(
                output, registry, issues, singleDoc, format, includeStats, sourcePath, testPath,
                includeIndices, includeSuppressInfo, includeExamples, includeSourceLinks, includeSeverityColor
            )
            generator.generate()
