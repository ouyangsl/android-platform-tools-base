...
class NoOpDetector : Detector(), SourceCodeScanner {
    ...
    companion object Issues {
        ...
        /**
         * Maximum number of indirect calls it will search when looking
         * for side effects
         */
         ...
         * of statements it will consider when looking for side effects
         */
        ...
        private fun hasSideEffect(node: UExpression?, depth: Int = 0, callDepth: Int = 0): Boolean {
            ...
            when (node) {
                ...
                is UTryExpression -> {
                    if (hasSideEffect(node.tryClause, depth + 1, callDepth) ||
                ...
                is UThisExpression, is USuperExpression -> return true
         ...
         * `isBar` ?
         *
         * We don't consider "get" by itself to be a getter name; it
         * needs to be a prefix for a named property.
         */
    ...
    private fun isPureMethod(context: JavaContext, node: UExpression, method: PsiMethod, methodName: String): Boolean {
        when (methodName) {
            // Integer.valueOf, etc
            "valueOf", "equals" -> return method.parameterList.parametersCount == 1
            "toString", "hashCode", "clone", "getText" -> return method.parameterList.parametersCount == 0
            ...
            "getComponent" -> if (method.isIn("com.intellij.execution.ui.RunnerLayoutUi", context)) { return false }
            "isActive" -> if (method.isIn("android.view.inputmethod.InputMethodManager", context)) { return false }
            "getDecorView" -> if (method.isIn("android.view.Window", context)) { return false }
            "getValue" -> if (method.isIn("androidx.compose.runtime.State", context) ||
            ...
            "getCount" -> if (method.isIn("android.database.Cursor", context)) { return false }
        }
        ...
        when (method.containingClass?.qualifiedName) {
            ...
            "kotlin.text.StringsKt__StringsKt" -> {
                ...
                if ((methodName == "getChars" || methodName == "getBytes") &&
    ...
    private fun isGetter(psiMethod: PsiMethod): Boolean {
        if (psiMethod.isConstructor || psiMethod.hasParameters() || psiMethod.returnType == PsiType.VOID) {
            return false
        }
